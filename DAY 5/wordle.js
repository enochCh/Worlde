const testWordList = ["abhor", "acerb", "bliss", "cynic", "deify", "ennui", "fjord", "gloze", "hokum", "junta", "knell", "limbo", "mirth", "nadir", "obese", "pique", "quell", "riven", "staid", "trite", "cheer", "fable", "grace", "humour", "jolly", "lofty", "merry", "plush", "silly", "witty"];
...
function startGame(round) {
    ...
    while (attemptCount <= round && status === "in-progress") {
        let currentGuess = prompt("Guess a five letter word, or die: ");
...
function retry(word) {
    alert(`${word} is failong his grads`);
}
...
async function loadOrStartGame(debug) {
    ...
    const prevGame = JSON.parse(window.localStorage.getItem("WORDLE"));
